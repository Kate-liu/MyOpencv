# -*- coding:utf-8 -*-
# This is a parking recognition TestData program
...
from ParkingSpaceRecognition.Parking import Parking
...
def img_process(test_images, park):
    ...
    with open('TestData/spot_dict.pickle', 'wb') as handle:
        pickle.dump(final_spot_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
    # If you want to check all parking space small picture, you can use these
...
def img_test(test_images, final_spot_dict, model, class_dictionary):
    for i in range(len(test_images)):
        predicted_images = parking.predict_on_image(test_images[i], final_spot_dict, model, class_dictionary)


if __name__ == '__main__':
    # read test image
    test_images_data = [plt.imread(path) for path in glob.glob("TestData/*.jpg")]
    # parking class
    parking = Parking()
    parking.show_images(test_images_data)
    # procession
    final_spot_dict = img_process(test_images_data, parking)

    # some parameters
    weights_path = "TrainResult/car-default.h5"
    class_dictionary = {0: 'empty', 1: 'occupied'}
    model = keras_model(weights_path)
    # keras predict
    img_test(test_images_data, final_spot_dict, model, class_dictionary)
